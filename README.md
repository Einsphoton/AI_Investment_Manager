...
- **资产管理**：支持股票、场内基金、场外基金持仓管理，展示市值、盈亏、交易记录与净值/价格走势。
- **标的池管理**：维护关注标的与 AI 推荐标的，支持优先级、风险等级、推荐后涨跌与分析浮窗。
- **行情与基本面**：接入新浪、腾讯、东方财富、天天基金、Yahoo 等数据源，覆盖报价、K 线、净值、基金信息与估值指标。
- **交互式图表**：净值曲线支持 1 个月到 10 年时间段；K 线支持 1D、5 日、日 K、周 K、月 K、季 K、年 K，并可缩放拖动。
- **后台预热与缓存**：页面打开后低优先级逐项预热行情、历史走势与基本面；详情页即时打开，真实数据后台刷新。
- **AI 分析工作流**：支持资产分析、标的分析、投资建议生成，以及 SSE 实时进度、日志和子任务状态展示。
- **OCR 导入**：可从资产截图或文件中识别持仓信息，辅助批量录入。
...
| 资产详情打开 | 等待行情/净值接口完成后展示 | 先展示本地数据，行情后台刷新 |
| 图表时间段切换 | 每次切换重新请求数据源 | 长周期缓存 + 本地裁剪/聚合 |
| 页面行情加载 | 进入页面后批量并发请求 | 低优先级逐项预热，不阻塞交互 |
